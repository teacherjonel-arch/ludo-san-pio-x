...
function broadcast(room,message){io.to(room.code).emit('state',publicState(room,message));io.to(room.code).emit('roomInfo',{code:room.code,playerCount:room.playerCount,started:room.started,host:room.host,players:room.players.map(p=>({id:p.id,team:p.team}))})}
...
io.on('connection',socket=>{
 socket.on('createRoom',d=>{
  ...
  const c=code();const room={code:c,host:socket.id,playerCount:n,players:[{id:socket.id,team:0}],started:false,current:0,value:null,awaitingMove:false,tokens:initialTokens(n),winner:null,questions:d.questions&&d.questions.questionMapVersion===2?d.questions:{customCellQuestions:{},skillQuestions:[]},qQueues:{},pending:null};
 ...
 socket.on('joinRoom',d=>{
  ...
  room.players.push({id:socket.id,team});socket.join(c);socket.data.room=c;socket.data.team=team;socket.emit('joinedRoom',{code:c,team,playerCount:room.playerCount});broadcast(room,'Jugador conectado.');
 });
